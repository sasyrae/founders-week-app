...
function buildHotelNote(config, a) {
  ...
  /* Founders get a hotel-issued discount code that unlocks their rate.
     Only shown to Founders, and only once a code has been set. */
  const code = isFounder && config.hotelFounderCode ? String(config.hotelFounderCode).trim() : "";
  const codePart = code ? ` (use code ${code} at booking)` : "";

  const base = `Hotel: you'll be staying at the host hotel The William Vale in Williamsburg for ${count} ${nightWord} (${dates}). We have negotiated a room rate of $${rate}/night${codePart}`;
