...
function speakerToRow(s) {
  ...
  // Only touch photo_url / sort_order when explicitly provided, so a text
  // edit never wipes an uploaded photo or a manual ordering.
  if (s.photoUrl !== undefined) row.photo_url = s.photoUrl || null;
...
async function ensureSpeakerBucket() {
  ...
  if (data) return;
  // public read; writes only ever happen server-side via the service role.
  await client.storage.createBucket(SPEAKER_BUCKET, {
    public: true,
    fileSizeLimit: "6MB",
